...
def render_embeddings(
    pred_denorm, targets, patient_ids, cardio_utils, embeddings_dir, prefix
):
    try:
    ...
    tsne = TSNE(
        n_components=2, random_state=42, perplexity=min(30, len(scaled_data) - 1)
    )
    ...
    embedding_data = {
        ...
        "patient_ids": (
            patient_ids if isinstance(patient_ids, list) else list(patient_ids)
        ),
    }
