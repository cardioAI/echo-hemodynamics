...
def generate_summary_report(
    ...
    with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
        if include_components:
            summary_data = {
                "Metric": [
                    ...
                    "Total Patients",
                    "Parameters Analyzed",
                    "Training Epochs (Latest)",
                    "Model Architecture",
                    "GPU Used",
                ],
                "Value": [
                    dataset_label,
                    len(patient_ids) if patient_ids else 0,
                    len(param_names),
                    actual_epochs,
                    "Multistage Transformer CardioAI",
                    "NVIDIA RTX 6000 Ada",
                ],
            }
        else:
        ...
        if pred_denorm is not None and targets is not None:
            ...
            mse_values = [
                np.mean((pred_denorm[:, i] - targets[:, i]) ** 2)
                for i in range(len(param_names))
            ]
            performance_data = {
        ...
        if include_components:
            ...
            pd.DataFrame(components_data).to_excel(
                writer, sheet_name="Analysis_Components", index=False
            )
