...
def render_heteroscedasticity_analysis(
        ...
        hetero_stats.append(
            {
                ...
                "Heteroscedastic": (
                    "Yes" if sd_ratio > 1.5 or bp_pvalue < 0.05 else "No"
                ),
            }
        )
    ...
    plt.title(
        "Heteroscedasticity Summary: Residual Variance Analysis Across Value Ranges",
        fontsize=14,
        fontweight="bold",
        pad=20,
    )
    ...
    cardio_utils.save_figure(
        fig_table, "test_heteroscedasticity_table", subdir="figures"
    )
    plt.close(fig_table)
