...
def render_plain_scatter(
    pred_denorm, targets, param_names, cardio_utils, color_manager, prefix
):
    for i, param_name in enumerate(param_names):
        ...
        ax.grid(False)
        plt.tight_layout()
        cardio_utils.save_figure(
            fig, f"{prefix}_scatter_plot_{param_name.lower()}", subdir="figures"
        )
        plt.close(fig)


def render_ph_stratified_scatter(
    pred_denorm, targets, param_names, cardio_utils, prefix
):
    ...
    for i, param_name in enumerate(param_names):
        ...
        if np.any(ph_positive):
            ax.scatter(
                true_param[ph_positive],
                pred_param[ph_positive],
                alpha=0.7,
                s=120,
                marker="o",
                color=param_color,
        ...
        if np.any(ph_negative):
            ax.scatter(
                true_param[ph_negative],
                pred_param[ph_negative],
                alpha=0.7,
                s=120,
                marker="s",
                color=param_color,
        ...
        ax.text(
            0.5,
            0.97,
            f"{prefix.title()}: {param_name} (r={corr:.3f})",
