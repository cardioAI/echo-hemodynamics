...
def render_simple_roc(
    ...
    for i, param_name in enumerate(param_names):
        ...
        ax.plot(
            fpr,
            tpr,
            color=color_manager.get_color(i),
            linewidth=2,
        ...
        cardio_utils.save_figure(
            fig, f"{prefix}_roc_curve_{param_name.lower()}", subdir="figures"
        )
        plt.close(fig)
...
def render_dual_roc(pred_denorm, targets, param_names, cardio_utils, prefix):
    ...
    for i, param_name in enumerate(param_names):
        ...
        ax.text(
            0.5,
            0.97,
            f"{prefix.title()}: {param_name} ROC Curve",
