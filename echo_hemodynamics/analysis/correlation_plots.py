...
def render_correlation_bar(
    pred_denorm, targets, param_names, cardio_utils, filename, title, color_manager=None
):
    correlations = [
        calculate_correlation(pred_denorm[:, i], targets[:, i])
    ...
    if color_manager is not None:
        ...
        colors = [
            (
                palette_colors[0]
                if c >= 0.6
                else palette_colors[1] if c > 0.3 else palette_colors[2]
            )
    ...
    if color_manager is not None:
        ax.axhline(
            y=0.6, color=threshold_color, linestyle="--", alpha=0.7, label="Target Min"
        )
        ax.legend()
...
def render_correlation_bubble_heatmap(
    pred_denorm, param_names, cardio_utils, filename, title
):
    fig, ax = plt.subplots(1, 1, figsize=(8, 8))
    corr_matrix = np.corrcoef(pred_denorm.T)
