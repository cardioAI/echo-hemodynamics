...
class CardioAIDataset(Dataset):
    ...
    def __init__(
        ...
        try:
            ...
            df_clean = df.dropna()
            print(
                f"After cleaning: {len(df_clean)} rows (removed {len(df) - len(df_clean)} with missing data)"
            )
        ...
        print("\nWinsorization:")
        print(
            f"{'Parameter':<10} {'Lower%':<8} {'Upper%':<8} {'#Lower':<8} {'#Upper':<8} {'Range'}"
        )
        print("-" * 70)
        ...
        for i, param_name in enumerate(parameter_names):
            ...
            labels[:, i], n_lower, n_upper, lower_bound, upper_bound = (
                winsorize_parameter(
                    labels[:, i], lower_percentile=lower_pct, upper_percentile=upper_pct
                )
            )
            if i in self.log_transform_indices:
                ...
                self.winsorized_param_maxs.append(upper_bound)
            print(
                f"{param_name:<10} {lower_pct:<8} {upper_pct:<8} {n_lower:<8} {n_upper:<8} [{lower_bound:.2f}, {upper_bound:.2f}]"
            )
        self.winsorized_param_mins = np.array(
            self.winsorized_param_mins, dtype=np.float32
        )
        self.winsorized_param_maxs = np.array(
            self.winsorized_param_maxs, dtype=np.float32
        )
        ...
        for idx, patient_id in enumerate(patient_ids):
            tensor_files = [
                self.tensor_dir / f"{patient_id}_{view}.pt" for view in self.views
            ]
            if all((tensor_file.exists() for tensor_file in tensor_files)):
                ...
                if not np.any(np.isnan(patient_labels)):
                        ...
                        try:
                            ...
                            for tensor_file in tensor_files:
                                ...
                                if tensor.shape[0] != max_frames:
                                    print(
                                        f"WARNING: Unexpected frame count {tensor.shape[0]} for {tensor_file}"
                                    )
                                    if tensor.shape[0] > max_frames:
                            ...
                            print(
                                f"Error loading tensors for patient {patient_id}: {e}"
                            )
                            continue
                    else:
                        self.data.append(
                    ...
                    print(
                        f"Skipping patient {patient_id}: Invalid labels (contains NaN)"
                    )
            else:
                missing_files = [f for f in tensor_files if not f.exists()]
                if len(missing_files) <= 2:
                    print(
                        f"Skipping patient {patient_id}: Missing {len(missing_files)} tensor files"
                    )
        ...
        if len(self.data) == 0:
            raise ValueError(
                "No valid patients found! Check tensor directory and Excel file."
            )
            ...
            for tensor_file in sample["tensor_files"]:
                try:
                    ...
                    if tensor.shape[0] != self.max_frames:
                        ...
                        if tensor.shape[0] > self.max_frames:
                            frame_indices = np.linspace(
                                0, tensor.shape[0] - 1, self.max_frames, dtype=int
                            )
                            tensor = tensor[frame_indices]
                        else:
    ...
    def print_dataset_info(self):
        ...
        print(
            f"\nDataset: {stats['total_patients']} patients, {len(self.views)} views, {self.max_frames} frames, {len(stats['parameter_names'])} parameters"
        )
        ...
        for param_name in stats["parameter_names"]:
            ...
            print(
                f"{param_name:<10} {s['mean']:<8.2f} {s['std']:<8.2f} {s['min']:<8.2f} {s['max']:<8.2f}"
            )
