...
def create_balanced_ph_splits(
    ...
    print(
        f"  PH Positive (>={threshold}): {len(positive_indices)} ({len(positive_indices) / len(df) * 100:.1f}%)"
    )
    print(
        f"  PH Negative (<{threshold}): {len(negative_indices)} ({len(negative_indices) / len(df) * 100:.1f}%)"
    )
    ...
    if strategy == "undersample_majority":
        ...
        train_pos_balanced = np.random.choice(
            train_pos_indices, min_class_size, replace=False
        )
        train_neg_balanced = np.random.choice(
            train_neg_indices, min_class_size, replace=False
        )
        ...
        train_pos_balanced = np.random.choice(
            train_pos_indices, max_class_size, replace=True
        )
        train_neg_balanced = np.random.choice(
            train_neg_indices, max_class_size, replace=True
        )
        train_indices = np.concatenate([train_pos_balanced, train_neg_balanced])
    ...
    print(
        f"  PH Positive: {info['train_positive']} ({info['train_positive'] / len(train_indices) * 100:.1f}%)"
    )
    print(
        f"  PH Negative: {info['train_negative']} ({info['train_negative'] / len(train_indices) * 100:.1f}%)"
    )
