...
def batch_convert(dcm_dir, output_dir, views=("FC", "TC", "SA", "LA")):
    ...
    for patient_idx, (patient_id, view_files) in enumerate(patient_views.items(), 1):
        ...
        for view in views:
            ...
            try:
                tensor = convert_dicom_to_tensor(
                    dcm_file, target_frames=32, target_size=(224, 224)
                )
...
def reconvert_external_patients():
    ...
    print("=" * 80)
    dcm_dir = "E:\\dataset_cardioAI\\EchoCath_cardioAI\\External_DCM"
    external_pt_dir = Path("E:\\dataset_cardioAI\\EchoCath_cardioAI\\External_PT")
    all_pt_dir = Path("E:\\dataset_cardioAI\\EchoCath_cardioAI\\All_PT")
    claude_dir = Path(
        "D:\\GoogleDrive\\Codes\\CardioAI\\CardioAI\\EchoCath_cardioAI\\Claude"
    )
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    backup_dir = Path(
        f"E:\\dataset_cardioAI\\EchoCath_cardioAI\\External_PT_backup_{timestamp}"
    )
