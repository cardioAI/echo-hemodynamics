...
def detect_fan_region_extreme_clean(gray_frame, visualize=False):
    ...
    if visualize:
        ...
        print(
            f"  Removed: top {top_crop}px, bottom {height - bottom_crop}px, left {left_crop}px, right {width - right_crop}px"
        )
...
def convert_dicom_to_tensor(
    dicom_path, target_frames=32, target_size=(224, 224), visualize=False
):
    if pydicom is None:
        raise ImportError(
    ...
    for idx, frame_idx in enumerate(frame_indices):
        ...
        fan_region = detect_fan_region_extreme_clean(
            gray_frame, visualize=visualize_this
        )
