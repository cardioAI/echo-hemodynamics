...
class CardioAIUtils:
    ...
    def _extract_palette_colors(self):
        try:
            ...
            print(
                f"Extracted {len(self.colors)} colors from palette (10 from palette + 8 grayscale)"
            )
    ...
    def get_color_palette(self, n_colors=None):
        if n_colors is None:
            ...
            for color in self.colors:
                if len(color) >= 3 and (
                    not (color[0] > 0.85 and color[1] > 0.85 and (color[2] > 0.85))
                ):
        ...
        for i, color in enumerate(safe_colors):
            if len(color) >= 3:
                ...
                if r > 0.9 and g > 0.9 and (b > 0.9):
                    print(
                        f"Warning: Color {i} might be too light: RGB({r:.3f}, {g:.3f}, {b:.3f})"
                    )

    def plot_lines_safe(
        self, ax, x_data, y_data_list, labels=None, linestyles=None, linewidths=None
    ):
    ...
    def setup_output_directory(self, timestamp=None, base_dir=None):
        if base_dir is None:
            base_dir = self.output_base_dir or "E:\\results_cardioAI\\EchoCath_cardioAI"
    ...
    def get_output_path(self, subdir_key, filename):
        if self.current_output_dir is None:
            raise ValueError(
                "Output directory not set. Call setup_output_directory() first."
            )
    ...
    def save_figure(
        self, fig, filename, subdir="figures", bbox_inches="tight", pad_inches=0.15
    ):
        if self.current_output_dir is None:
            raise ValueError(
                "Output directory not set. Call setup_output_directory() first."
            ...
            try:
                for fmt, ext in formats:
                    ...
                    try:
                        ...
                        print(
                            f"Warning: Could not save {filepath}: {e}", file=old_stderr
                        )
            finally:
        ...
        if name == "blue_cyan_yellow":
            if self.colors is not None and len(self.colors) >= 10:
                ...
                return mcolors.LinearSegmentedColormap.from_list(
                    "palette_10_colors", palette_10_colors
                )
            fallback_colors = [
                [0.12, 0.27, 0.44],
                [0.22, 0.4, 0.58],
            ...
            return mcolors.LinearSegmentedColormap.from_list(
                "blue_cyan_yellow_palette", fallback_colors
            )
            ...
            return mcolors.LinearSegmentedColormap.from_list(
                "blue_gray_orange", fallback_colors
            )
    ...
    def get_view_color_map(self):
        ...
        view_colors = [self.colors[5], dark_colors[0], self.colors[7], self.colors[9]]
