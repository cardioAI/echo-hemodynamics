...
class ProgressiveTrainer:
    ...
    def _create_stage_optimizer(self):
        return create_progressive_optimizer(
            self.model, task_lr=self.task_lr, vit_lr=self.vit_lr
        )
    ...
    def _advance_stage(self):
        ...
        if self.stages > 0 and self.current_stage <= self.stages:
            ...
            print(
                f"Unfreezing ViT transformer block {block_idx} (from last, max {self.stages} blocks)"
            )
            print(f"{'=' * 60}")
    ...
    def compute_correlations(self, predictions, targets):
        ...
        for i in range(predictions.shape[1]):
            ...
            if np.std(pred_i) > 1e-08 and np.std(targ_i) > 1e-08:
    ...
    def save_checkpoint(self, is_best=False):
        ...
        if is_best:
            try:
                ...
                print(
                    f"Saved best model with validation loss: {self.best_val_loss:.4f}"
                )
            except Exception as e:
    ...
    def train(self, total_epochs):
        ...
        print(f"{'=' * 80}")
        print(
            f"Epochs: {total_epochs}, Stage epochs: {self.stage_epochs}, Stages: {self.total_stages}"
        )
        ...
        for epoch in range(total_epochs):
            ...
            print(
                f"Epoch {epoch + 1:3d}/{total_epochs} | Stage {self.current_stage:2d} | Train Loss: {train_loss:.4f} | Int Val Loss: {val_loss:.4f} | Int Val Corr: {avg_corr:.3f} ({above_threshold}/9 >=0.6)"
            )
            if test_corr is not None:
                test_avg_corr = np.mean(test_corr)
                test_above_threshold = sum((1 for c in test_corr if c >= 0.6))
                print(
                    f"{'':>18} Test Loss: {test_loss:.4f} | Test Corr: {test_avg_corr:.3f} ({test_above_threshold}/9 >=0.6) | Time: {epoch_time:.1f}s"
                )
            else:
