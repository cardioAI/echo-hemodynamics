...
def run_cross_validation(
    ...
    print(
        f"  Cohort I  (training + {num_folds}-fold CV): {len(train_cv_indices)} patients"
    )
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(train_cv_indices)):
        ...
        fold_corr = (
            trainer.best_correlations if trainer.best_correlations else [0.0] * 9
        )
        fold_test_corr = (
            trainer.best_test_correlations
            if trainer.best_test_correlations
            else [0.0] * 9
        )
    ...
    cv_summary = {
        ...
        "per_fold_val_correlations": [
            list(map(float, fc)) for fc in all_fold_correlations
        ],
        "per_fold_test_correlations": [
            list(map(float, fc)) for fc in all_fold_test_correlations
        ],
        "avg_val_correlations": {
            p: float(c) for (p, c) in zip(param_names, avg_val_corr)
        },
        "std_val_correlations": {
            p: float(c) for (p, c) in zip(param_names, std_val_corr)
        },
        "avg_test_correlations": {
            p: float(c) for (p, c) in zip(param_names, avg_test_corr)
        },
        "std_test_correlations": {
            p: float(c) for (p, c) in zip(param_names, std_test_corr)
        },
    }
