...
def run_ablation_study():
    ...
    epochs = int(
        os.environ.get(
            "CARDIOAI_ABLATION_EPOCHS", os.environ.get("CARDIOAI_EPOCHS", 100)
        )
    )
    ...
    if not output_dir.exists():
        raise RuntimeError(
            f"Output directory {output_dir} should be created by main.py first"
        )
    cardio_utils = CardioAIUtils()
    cardio_utils.current_output_dir = output_dir
    cardio_utils.subdirs = {"figures": output_dir}
    print("\nLoading dataset...")
    tensor_dir = Path("E:\\dataset_cardioAI\\EchoCath_cardioAI\\All_PT")
    ...
    print(
        f"Normalization parameters: log_transform_indices={norm_params['log_transform_indices']}"
    )
    ...
    print(
        f"Dataset: {len(train_dataset)} train, {len(val_dataset)} validation (fold 1/{num_folds})"
    )
    print(
        f"  Validation PH distribution: {val_ph_positive} PH+ / {val_ph_negative} PH-"
    )
    ...
    for variant_name, model in variants.items():
        ...
        if variant_name == "full_model":
            ...
            trainer = ProgressiveAblationTrainer(
                model, train_loader, val_loader, device, epochs=1
            )
    ...
    for variant_name, result in results.items():
        ...
        summary["results"][variant_name] = {
            ...
            "per_param_correlations": {
                param_names[i]: reordered[i] for i in range(len(param_names))
            },
