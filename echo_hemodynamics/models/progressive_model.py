...
class ProgressiveCardioAI(ExplainabilityMixin, nn.Module):
    ...
    def __init__(
        ...
        if self.ablation_config == "none":
            ...
            self.use_fusion_attention = True
        try:
            self.vision_transformer = ViTModel.from_pretrained(
                "google/vit-base-patch16-224"
            )
            print(
                f"Loaded ViT-Base: hidden={self.vision_transformer.config.hidden_size}, heads={self.vision_transformer.config.num_attention_heads}, layers={self.vision_transformer.config.num_hidden_layers}"
            )
        ...
        self.register_buffer(
            "param_mins",
            torch.tensor([0.0, 18.0, 5.0, 10.0, 1.0, 2.0, 1.2, 3.85, 0.56]),
        )
        self.register_buffer(
            "param_maxs",
            torch.tensor([22.0, 111.0, 75.0, 71.0, 25.0, 10.0, 5.5, 5576.0, 19.74]),
        )
        self.log_transform_indices = [7]
    ...
    def _init_task_modules(self):
        modules_to_init = (
            [self.temporal_attention] if self.temporal_attention is not None else []
        )
    ...
    def freeze_vit_backbone(self):
        ...
        print(
            f"Parameters: {trainable_params:,} trainable / {total_params:,} total ({100 * trainable_params / total_params:.1f}%)"
        )

    def unfreeze_vit_block(self, block_idx):
        if hasattr(self.vision_transformer, "encoder") and hasattr(
            self.vision_transformer.encoder, "layer"
        ):
            if block_idx < len(self.vision_transformer.encoder.layer):
                for param in self.vision_transformer.encoder.layer[
                    block_idx
                ].parameters():
                    param.requires_grad = True
                ...
                print(
                    f"Warning: Block {block_idx} does not exist (max: {len(self.vision_transformer.encoder.layer) - 1})"
                )
        else:
            print("Warning: Could not find encoder layers in ViT model structure")

    def set_winsorized_normalization(self, norm_params):
        self.param_mins = torch.tensor(
            norm_params["param_mins"], dtype=torch.float32
        ).to(self.param_mins.device)
        self.param_maxs = torch.tensor(
            norm_params["param_maxs"], dtype=torch.float32
        ).to(self.param_maxs.device)
        if len(norm_params["log_mins"]) > 0:
            self.log_mins = torch.tensor(
                norm_params["log_mins"], dtype=torch.float32
            ).to(self.log_mins.device)
            self.log_maxs = torch.tensor(
                norm_params["log_maxs"], dtype=torch.float32
            ).to(self.log_maxs.device)
    ...
    def get_trainable_parameters(self):
        ...
        vit_params = [
            p for p in self.vision_transformer.parameters() if p.requires_grad
        ]
    ...
    def normalize_targets(self, targets):
        ...
        for i in range(self.num_outputs):
            if i not in self.log_transform_indices:
                normalized[:, i] = (targets[:, i] - self.param_mins[i]) / (
                    self.param_maxs[i] - self.param_mins[i] + 1e-08
                )
        for i, param_idx in enumerate(self.log_transform_indices):
            log_values = torch.log(targets[:, param_idx] + 1)
            normalized[:, param_idx] = (log_values - self.log_mins[i]) / (
                self.log_maxs[i] - self.log_mins[i] + 1e-08
            )
        return normalized

    def denormalize_predictions(self, predictions):
        predictions = torch.clamp(predictions, min=1e-07, max=1.0 - 1e-07)
        denormalized = torch.zeros_like(predictions)
        for i in range(self.num_outputs):
            if i not in self.log_transform_indices:
                denormalized[:, i] = (
                    predictions[:, i] * (self.param_maxs[i] - self.param_mins[i])
                    + self.param_mins[i]
                )
        for i, param_idx in enumerate(self.log_transform_indices):
            log_values = (
                predictions[:, param_idx] * (self.log_maxs[i] - self.log_mins[i])
                + self.log_mins[i]
            )
    ...
    def forward(self, views, return_aux=False):
        ...
        for view_idx, view in enumerate(views):
            ...
            if self.use_temporal_attention and self.temporal_attention is not None:
                attended_view, temp_att, frame_weights = self.temporal_attention(
                    frame_features
                )
        ...
        parameter_predictions = [
            head(unified_features) for head in self.regression_heads
        ]
