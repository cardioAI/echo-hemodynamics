...
def _find_model_path(output_base_dir):
    candidates = [
        os.path.join(
            output_base_dir, "..", "attention_ablation_cardioAI", "best_model.pth"
        ),
        os.path.join(
            output_base_dir,
            "..",
            "attention_ablation_cardioAI",
...
def main():
    try:
        ...
        dataset_dir = "E:\\dataset_cardioAI\\EchoCath_cardioAI\\All_PT"
        excel_file = "./All.xlsx"
        ...
        print(
            f"Using {n_frames} frames with highest attention importance for visualizations"
        )
        overlay_alpha_full = float(os.environ.get("CARDIOAI_OVERLAY_ALPHA_FULL", 0.7))
        overlay_alpha_selective = float(
            os.environ.get("CARDIOAI_OVERLAY_ALPHA_SELECTIVE", 0.8)
        )
