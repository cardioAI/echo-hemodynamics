...
from ..analysis.correlation_plots import (
...
from ..figures.training_curves import (
    generate_training_figures as _generate_training_figures,
)
...
def _load_training_history(current_timestamp):
    base_dir = Path("E:\\results_cardioAI\\EchoCath_cardioAI")
    ...
    if base_dir.exists():
        for result_dir in sorted(
            [d for d in base_dir.glob("*") if d.is_dir()], reverse=True
        ):
...
def main():
    try:
        ...
        tensor_dir = "E:\\dataset_cardioAI\\EchoCath_cardioAI\\All_PT"
        excel_file = "./All.xlsx"
        ...
        if missing_keys or unexpected_keys:
            print(
                f"Warning: Model architecture mismatch. Missing: {len(missing_keys)}, Unexpected: {len(unexpected_keys)}"
            )
        ...
        render_plain_scatter(
            pred_denorm, targets, PARAM_NAMES, cardio_utils, cardio_utils, "validation"
        )
        render_plain_bland_altman(
            pred_denorm, targets, PARAM_NAMES, cardio_utils, cardio_utils, "validation"
        )
        render_simple_roc(
            pred_denorm, targets, PARAM_NAMES, cardio_utils, cardio_utils, "validation"
        ...
        render_confusion_matrices(
            pred_denorm, targets, PARAM_NAMES, cardio_utils, "validation"
        )
        render_embeddings(
            pred_denorm,
            targets,
