...
class CardioAIPipeline:
    ...
    def __init__(self, base_output_dir=None):
        if base_output_dir is None:
            base_output_dir = "E:\\results_cardioAI\\EchoCath_cardioAI"
        ...
        self.dirs = {
            key: self.run_dir / name for (key, name) in STAGE_DIR_NAMES.items()
        }
    ...
    def log_step(self, step, status, details="", duration=None):
        ...
        status_symbol = (
            "OK" if status == "SUCCESS" else "ERROR" if status == "ERROR" else "RUNNING"
        )
    ...
    def find_and_copy_latest_weights(self):
        ...
        if not self.base_output_dir.exists():
            raise FileNotFoundError(
                f"Results directory not found: {self.base_output_dir}"
            )
        timestamp_dirs = [
            d
            for d in self.base_output_dir.iterdir()
            if d.is_dir() and d.name != self.timestamp
        ]
        ...
        with open(readable_file, "w") as f:
            ...
            for entry in self.pipeline_log:
                status_symbol = (
                    ...
                    else "ERROR" if entry["status"] == "ERROR" else "RUNNING"
                )
                duration = (
                ...
                f.write(
                    f"{status_symbol} {entry['step']}: {entry['status']}{duration}\n"
                )
        ...
        try:
            try:
                ...
                if torch.cuda.is_available():
                    ...
                    print(
                        f"GPU Memory: {torch.cuda.get_device_properties(0).total_memory / 1000000000.0:.1f} GB"
                    )
            except ImportError:
                print(
                    "Note: PyTorch not available in this environment; GPU setup skipped."
                )
            ...
            if self.skip_train:
                ...
                try:
                    ...
                    self.log_step(
                        "load_latest_weights", "SUCCESS", f"Loaded from {latest_run}"
                    )
            ...
            if not self.skip_ablation:
                ...
                if not success:
                    print(
                        f"WARNING: Ablation study failed: {(stderr[-200:] if stderr else 'None')}"
                    )
            else:
            ...
            if not self.skip_visualizations:
                ...
                if not success:
                    print(
                        f"ERROR: Visualizations failed: {(stderr[-300:] if stderr else 'None')}"
                    )
            else:
            ...
            if self.run_validation:
                ...
                if not success:
                    print(
                        f"WARNING: Validation failed: {(stderr[-200:] if stderr else 'None')}"
                    )
            if self.run_test:
                print(f"\n{'=' * 50}")
                ...
                if not success:
                    print(
                        f"WARNING: Test failed: {(stderr[-200:] if stderr else 'None')}"
                    )
            self.save_pipeline_summary()
            print(f"\n{'=' * 60}")
            ...
            successful = sum(
                (1 for entry in self.pipeline_log if entry["status"] == "SUCCESS")
            )
...
def build_parser():
    ...
    parser.add_argument(
        "--run-validation", dest="run_validation", action="store_true", default=True
    )
    parser.add_argument("--no-validation", dest="run_validation", action="store_false")
    parser.add_argument(
        "--run-test", dest="run_test", action="store_true", default=True
    )
